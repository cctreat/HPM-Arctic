function rmseErr = hpm20_mon_mainOptim(x) 
% function to optimize veg and runoff
% start runon/runoff/ height = x(1), ...
% x(2) = Fen-bog transition, shift in anoxia scale length;
% x(3) = relative NPP of shrubs;
% anoxia_scalelengthFen = x(4); 
% anoxia_scalelengthBog = x(5);
...
[params.WTD_opt, params.WTD_range, params.ALD_opt, params.ALD_range,...
    params.NPP_rel, params.max_npp, params.q10_npp, ...
    params.ag_frac_npp, params.bg_frac_npp, params.k_0, params.k_month_0] = hpm20_vegOptim(x, params);
...
% find height where veg switches from sedge to dominant moss & shrubs
sedgeDomTF = (mfrac(:, 2) + mfrac(:, 3)) < 0.4; % identifies sedge-"dominant" cohorts
y1 = find(~smooth(sedgeDomTF, 30),1); % finds last occurent of 30-year mean sedge dominance
modH = max(depth) - depth(y1);


rmseErr = rmse_modData(results_1(modObs_ageIndex, 2)*100, params.obsCore_depths.') ;
%+ ...
 %   rmse_modData(modH*100, params.obs_sedgeOff_ht*100)/2;
return
