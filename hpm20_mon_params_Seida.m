...
site_name = 'Seida';
sim_name = '_HeatIn32';
monthly_T_P_name =  '_monthly_T_P_2500BP_2100CE'; 
...
sim_start = 2500; % years BP (before 'present'), where 0 PB = 1950 CE
...
ann_temp = -6;  % site mean annual temp (C) for parameter values
ann_ppt = 0.35;  % site total annual precipitation (m/y) for parameter values
ann_ET_0 = 0.35;  % site base evapotranspiration (m/y) used to compute base run-off
% ann_ET_0 = 0.281*exp(0.068 * ann_temp);  % from Julie Talbot; update?

latitude = 67; % degrees North > 0
...
HeatFlux_StartYear = 1000;  % first year of simulation with lateral heat flux 
...
elseif (bog_fen_id < 2.5)   %  PERENNIAL FEN VALUES
    ...
    Roff_c1 = max(0,ann_ppt - ann_ET_0 + 0.05); % max runoff + max ET = mean annual precip + xx m/yr
    Roff_c2 = 0.2;  % linear increase in runoff (m/y) per meter of total peat height
    Roff_c2a = 1.;  % peat height needed to get base run-off (factor = 1 +c2*(H-c2a))
    Roff_c2a = 1.2 * start_depth;  % peat height needed to get base run-off (factor = 1 +c2*(H-c2a))
    ...
    runon_c1 = 1.0;  % total peat depth (m) at which run-on declines by ~50%
    runon_c2 = 0.5;  % controls rate of decline of run-on as function of peat height (see 'HPM vegetation productivity.xls')
    runon_c3 = 0.5;  % magnitude of maximum run-on (m/month)
...
max_inundation = 0.03;  % 0.1; % max. water pooling depth (m) ABOVE peat surface
...
%     'temp_flag','temp_rand_persist','temp_amp1','temp_amp2',...
%     'ppt_flag','ppt_amp1','ppt_amp2','ppt_rand_persist','mon_ppt_offset',...
