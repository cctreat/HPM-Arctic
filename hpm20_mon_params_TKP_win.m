...
site_name = 'TKP';
sim_name = '_2C_dry_2300';
monthly_T_P_name =  '_monthly_T_P_6620BP_2300CE'; 
...
sim_start = 6620; % years BP (before 'present'), where 0 BP = 1950 CE
...
ann_temp = -11;  % site mean annual temp (C) for parameter values
ann_ppt = 0.272;  % site total annual precipitation (m/y) for parameter values
ann_ET_0 = 0.27;  % site base evapotranspiration (m/y) used to compute base run-off

latitude = 66.5; % degrees North > 0
...
depth_runOnOff = 0.5;% depth when run-on switches to runoff
depth_MnOmTrans = 2.1; %depth of the transition from minerotrophy to ombrotrophy.
...
%     Roff_c2a = 1.;  % peat height needed to get base run-off (factor = 1 +c2*(H-c2a))
    ...
    anoxia_scale_length = 3;
    anoxia_scale_length1 = anoxia_scale_length;
    anoxia_scale_length2 = 1.2;
